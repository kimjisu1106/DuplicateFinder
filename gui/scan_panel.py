...
class ScanPanel(tk.LabelFrame):
    ...
    def _build(self):
        ...
        tk.Button(row0, text=t('btn_choose_folder'), command=self._choose_folder, width=10).pack(side='left')
        ...
        tk.Checkbutton(row1, text=t('cb_include_subfolders'), variable=self._recursive_var).pack(side='left')

        tk.Label(row1, text=t('label_search_target')).pack(side='left')
        ...
        tk.Label(self._threshold_frame, text=t('label_sensitivity')).pack(side='left')
        ...
        tk.Label(self._threshold_frame, text=t('label_sensitivity_hint'), foreground='#888888',
                 font=(APP_FONT_FAMILY, APP_FONT_SIZE - 1)).pack(side='left', padx=4)

        bmc = tk.Label(row1, text=t('label_sponsor'), cursor='hand2',
        ...
        self._scan_btn = tk.Button(row2, text=t('btn_scan_start'), command=self._start_scan,
                                   width=12, bg='#4CAF50', fg='white',
                                   font=(APP_FONT_FAMILY, APP_FONT_SIZE, 'bold'))
        self._scan_btn.pack(side='left', padx=(0, 6))

        self._pause_btn = tk.Button(row2, text=t('btn_pause'), command=self._toggle_pause,
                                    width=10, state='disabled')
        self._pause_btn.pack(side='left', padx=(0, 6))

        self._cancel_btn = tk.Button(row2, text=t('btn_cancel_scan'), command=self._cancel_scan,
    ...
    def update_progress(self, current: int, total: int, filename: str = ''):
        ...
        self._count_var.set(t('status_progress_count', current=f'{current:,}', total=f'{total:,}', pct=pct))
