...
- 스캔 중에는 폴더 선택 / 옵션 체크박스 / 언어 전환 버튼 전체 비활성화 → 스캔 종료 시 자동 복원
- 스캔 완료 후 언어 전환 시 결과 사라짐 경고 다이얼로그 표시 (`MainWindow._has_results` 플래그)
- 모든 `Toplevel` 다이얼로그(삭제 중 / 처리 중 / 폴더 선택)에 앱 아이콘 일괄 적용
